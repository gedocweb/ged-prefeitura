 ...
 * 
 *
 * @param <T> deve ser referência de uma entidade filha da {@link EntidadeBasicaAudit}
 * @param <ID> deve ser a chave primária da entidade (Objeto que implemente {@link Serializable})
 */

@Stateless
public class GenericServiceControllerAudit<T extends EntidadeBasicaAudit, ID extends Serializable> {
	
	/**
	 * Possui os mesmos métodos desta classe, mas sem a opção do Interceptor para enviar mensagens para tela do usuário.
	 */
	@EJB
	private GenericServiceAudit<T, Serializable> genericService;
